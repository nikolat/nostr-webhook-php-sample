...
function talk($data, $emojiTags, $rootTag, $isMentionOther, $mentionOtherTag) {
	...
	else if (preg_match('/(.+)[をに]([燃萌も]やして|焼いて|凍らせて|冷やして|通報して|火を[付つ]けて|磨いて).?$/us', $content, $match)) {
...
function airrep($data, $emojiTags) {
	...
	else if (preg_match('/(.{1,30})[をに]([燃萌も]やして|焼いて|凍らせて|冷やして|通報して|火を[付つ]けて|磨いて).?$/us', $content, $match)) {
		$target = $match[1];
		$lines = preg_split("/\r\n|\r|\n/", $match[1]);
		$len = 0;
		$len_max = 0;
